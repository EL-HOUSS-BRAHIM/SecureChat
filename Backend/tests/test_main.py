...
def test_user_registration(mock_db):
    ...
    assert response.json() == {"msg": "User registered successfully"}, "Unexpected response message"
...
def test_user_login(mock_db):
    ...
    response = client.post("/login", json={"email": "test@example.com", "password": "password123", "totp_code": ""})
    assert response.status_code == 200, f"Login failed with status code {response.status_code}"
    assert response.json() == {"msg": "Login successful"}, "Unexpected login response message"
...
def test_user_logout():
    ...
    assert response.json() == {"msg": "User logged out successfully"}, "Unexpected logout response message"
    print("User logout test passed successfully")

# Friends Tests
def test_send_friend_request(mock_db):
    response = client.post("/friends/request", json={"requester_id": 1, "receiver_ms_id": "receiver123"})
    assert response.status_code == 200, f"Sending friend request failed with status code {response.status_code}"
    assert response.json() == {"msg": "Friend request sent"}, "Unexpected friend request response message"
    print("Send friend request test passed successfully")

def test_accept_friend_request(mock_db):
    client.post("/friends/request", json={"requester_id": 1, "receiver_ms_id": "receiver123"})
    response = client.post("/friends/accept", json={"request_id": 1})
    assert response.status_code == 200, f"Accepting friend request failed with status code {response.status_code}"
    assert response.json() == {"msg": "Friend request accepted"}, "Unexpected friend request acceptance response message"
...
async def test_send_message(mock_db):
    ...
    websocket.receive_json.return_value = {"content": "Hello", "receiver_public_key": "dummy_key"}
...
def test_retrieve_messages(mock_db):
    ...
    assert response.status_code == 200, f"Retrieving messages failed with status code {response.status_code}"
...
def test_update_ms_id(mock_db):
    client.post("/register", json={"username": "testuser", "email": "test@example.com", "password": "password123"})
    response = client.post("/profile/update-ms_id", json={"user_id": 1, "new_ms_id": "newmsid123"})
    assert response.status_code == 200, f"Updating ms_id failed with status code {response.status_code}"
    assert response.json() == {"msg": "ms_id updated"}, "Unexpected ms_id update response message"
...
def test_enable_2fa(mock_db):
    ...
    assert response.status_code == 200, f"Enabling 2FA failed with status code {response.status_code}"
    assert response.json() == {"msg": "2FA enabled"}, "Unexpected 2FA enable response message"
...
def test_upload_media():
    with patch("app.core.encryption.encrypt_image", return_value=b"encrypted_image_data"):
        response = client.post("/media/upload", files={"file": ("test.jpg", b"image_data", "image/jpeg")})
    assert response.status_code == 200, f"Media upload failed with status code {response.status_code}"
    assert response.json() == {"msg": "File uploaded successfully"}, "Unexpected media upload response message"
...
def test_retrieve_media():
    ...
    assert response.status_code == 200, f"Media retrieval failed with status code {response.status_code}"
...
def test_create_session(session_manager):
    ...
    assert "session_token" in session_manager.active_sessions[user_id], "Session token not generated"
...
def test_revoke_all_sessions(session_manager):
    ...
    assert result == {"msg": "All sessions revoked"}, "Unexpected response message for revoking all sessions"
...
def test_check_session_valid(session_manager):
    ...
    assert session_manager.check_session(user_id, session_token) is True, "Valid session check failed"
...
def test_full_user_flow(mock_db):
    # Register
    register_response = client.post("/register", json={"username": "testuser", "email": "test@example.com", "password": "password123"})
    assert register_response.status_code == 200, f"Registration failed with status code {register_response.status_code}"
    print("User registration in full flow test passed successfully")

    # Login
    login_response = client.post("/login", json={"email": "test@example.com", "password": "password123", "totp_code": ""})
    assert login_response.status_code == 200, f"Login failed with status code {login_response.status_code}"
    print("User login in full flow test passed successfully")

    # Enable 2FA
    enable_2fa_response = client.post("/profile/enable-2fa", json={"user_id": 1})
    assert enable_2fa_response.status_code == 200, f"Enabling 2FA failed with status code {enable_2fa_response.status_code}"
    print("Enable 2FA in full flow test passed successfully")

    # Send friend request
    send_request_response = client.post("/friends/request", json={"requester_id": 1, "receiver_ms_id": "receiver123"})
    assert send_request_response.status_code == 200, f"Sending friend request failed with status code {send_request_response.status_code}"
    print("Send friend request in full flow test passed successfully")

    # Accept friend request
    accept_request_response = client.post("/friends/accept", json={"request_id": 1})
    assert accept_request_response.status_code == 200, f"Accepting friend request failed with status code {accept_request_response.status_code}"
    print("Accept friend request in full flow test passed successfully")

    # Upload and retrieve media
    with patch("app.core.encryption.encrypt_image", return_value=b"encrypted_image_data"):
        upload_media_response = client.post("/media/upload", files={"file": ("test.jpg", b"image_data", "image/jpeg")})
    assert upload_media_response.status_code == 200, f"Media upload failed with status code {upload_media_response.status_code}"
    print("Upload media in full flow test passed successfully")

    with patch("app.core.encryption.decrypt_image", return_value=b"decrypted_image_data"):
        retrieve_media_response = client.get("/media/retrieve/1")
    assert retrieve_media_response.status_code == 200, f"Media retrieval failed with status code {retrieve_media_response.status_code}"
...
def test_registration_with_existing_email(mock_db):
    ...
    response = client.post("/register", json={"username": "testuser2", "email": "test@example.com", "password": "password456"})
    assert response.status_code == 400, f"Expected 400 status code, but got {response.status_code}"
    assert "Email already registered" in response.json()["detail"], "Unexpected error message for duplicate email registration"
...
def test_login_with_incorrect_password(mock_db):
    ...
    response = client.post("/login", json={"email": "test@example.com", "password": "wrongpassword", "totp_code": ""})
    assert response.status_code == 401, f"Expected 401 status code, but got {response.status_code}"
    assert "Incorrect email or password" in response.json()["detail"], "Unexpected error message for incorrect password"
...
def test_friend_request_to_non_existent_user(mock_db):
    ...
    response = client.post("/friends/request", json={"requester_id": 1, "receiver_ms_id": "non_existent_user"})
    assert response.status_code == 404, f"Expected 404 status code, but got {response.status_code}"
    assert "User not found" in response.json()["detail"], "Unexpected error message for friend request to non-existent user"
...
def test_accept_non_existent_friend_request(mock_db):
    ...
    assert response.status_code == 404, f"Expected 404 status code, but got {response.status_code}"
    assert "Friend request not found" in response.json()["detail"], "Unexpected error message for accepting non-existent friend request"
...
async def test_send_message_to_non_friend(mock_db):
    ...
    websocket = await mock_websocket()
    websocket.receive_json.return_value = {"content": "Hello", "receiver_public_key": "dummy_key", "receiver_id": 999}
    
    with pytest.raises(HTTPException) as exc_info:
        await app.send_message(websocket)
    
    assert exc_info.value.status_code == 403, f"Expected 403 status code, but got {exc_info.value.status_code}"
    assert "Cannot send message to non-friend" in str(exc_info.value.detail), "Unexpected error message for sending message to non-friend"
...
def test_retrieve_messages_with_invalid_ids(mock_db):
    ...
    assert response.status_code == 404, f"Expected 404 status code, but got {response.status_code}"
    assert "No messages found" in response.json()["detail"], "Unexpected error message for retrieving messages with invalid IDs"
    print("Retrieve messages with invalid IDs test passed successfully")

def test_update_ms_id_for_non_existent_user(mock_db):
    response = client.post("/profile/update-ms_id", json={"user_id": 999, "new_ms_id": "newmsid123"})
    assert response.status_code == 404, f"Expected 404 status code, but got {response.status_code}"
    assert "User not found" in response.json()["detail"], "Unexpected error message for updating ms_id of non-existent user"
...
def test_enable_2fa_for_non_existent_user(mock_db):
    ...
    assert response.status_code == 404, f"Expected 404 status code, but got {response.status_code}"
    assert "User not found" in response.json()["detail"], "Unexpected error message for enabling 2FA for non-existent user"
    print("Enable 2FA for non-existent user test passed successfully")

def test_upload_invalid_media_type():
    response = client.post("/media/upload", files={"file": ("test.txt", b"text data", "text/plain")})
    assert response.status_code == 400, f"Expected 400 status code, but got {response.status_code}"
    assert "Invalid file type" in response.json()["detail"], "Unexpected error message for uploading invalid media type"
...
def test_retrieve_non_existent_media():
    ...
    assert response.status_code == 404, f"Expected 404 status code, but got {response.status_code}"
    assert "Media not found" in response.json()["detail"], "Unexpected error message for retrieving non-existent media"
...
def test_encrypt_decrypt_message_mismatch():
    ...
    assert "Decryption failed" in str(exc_info.value), "Unexpected error message for decryption with mismatched key"
...
def test_invalid_totp():
    ...
    assert not verify_totp(secret, invalid_totp), "Invalid TOTP should not be verified"
...
async def test_websocket_disconnect_handling():
    ...
    await manager.connect(mock_websocket)
    manager.disconnect(mock_websocket)
    assert len(manager.active_connections) == 0, "WebSocket connection not properly removed after disconnection"
...
def test_session_expiration(session_manager):
    ...
    assert exc_info.value.status_code == 401, f"Expected 401 status code, but got {exc_info.value.status_code}"
    assert "Session expired" in str(exc_info.value.detail), "Unexpected error message for expired session"
...
def test_email_service_failure(mock_smtp):
    ...
    assert "Failed to send email" in str(exc_info.value), "Unexpected error message for email service failure"
...
def test_full_user_flow_with_2fa(mock_db):
    ...
    login_response_no_2fa = client.post("/login", json={"email": "test@example.com", "password": "password123", "totp_code": ""})
    assert login_response_no_2fa.status_code == 401, f"Expected 401 status code for login without 2FA, but got {login_response_no_2fa.status_code}"
    print("Login attempt without 2FA code test passed successfully")

    # Login with 2FA code
    with patch("app.core.security.verify_totp", return_value=True):
        login_response_with_2fa = client.post("/login", json={"email": "test@example.com", "password": "password123", "totp_code": "123456"})
    assert login_response_with_2fa.status_code == 200, f"Login with 2FA failed with status code {login_response_with_2fa.status_code}"
